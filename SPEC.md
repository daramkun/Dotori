...
- [ ] `ToolchainInfo`에 `IsClangCl` 계산 속성 추가
  - `Kind == Msvc && Path.GetFileNameWithoutExtension(CompilerPath) == "clang-cl"`
- [ ] `ToolchainDetector.TryGetMsvcPaths(arch)` 헬퍼 메서드 추출
  - 기존 `TryFindMsvc()` 내부 로직에서 MSVC 경로 수집 부분 분리
- [ ] `TryFindClangWindows()` 수정
...
- [ ] 단위 테스트: `IsClangCl == true`일 때 `-imsvc` 플래그가 포함되는지 검증

#### 2. MinGW 크로스 컴파일 (비-Windows 호스트 → Windows 타겟)

- [ ] `ToolchainInfo`에 `IsMinGW` 계산 속성 추가
  - `Kind == Clang && TargetTriple.Contains("mingw")`
- [ ] `ToolchainDetector.DetectWindows()` 수정
  - Windows 호스트가 아닌 경우 → `DetectWindowsCross(arch)` 호출
- [ ] `ToolchainDetector.DetectWindowsCross(string arch)` 추가
...
- [ ] `ToolchainDetector.FindMinGWSysroot(string arch)` 추가
  - `MINGW_SYSROOT` 환경변수 → Homebrew 경로 → Linux `/usr/<triple>` 순으로 탐색
- [ ] `BuildPlanner.Link.cs` 수정
...
- [ ] 자동완성 (`CompletionProvider`, `textDocument/completion`)
...
- [ ] 호버 (`HoverProvider`, `textDocument/hover`)
  - DSL 키워드에 대한 한줄 설명 (Markdown)
  - 열거형 값 설명 (예: `musl` → "완전 정적 바이너리 가능, Linux 전용")
  - `path` 의존성 hover → 대상 프로젝트 이름 + 타입 표시

- [ ] 정의로 이동 (`DefinitionProvider`, `textDocument/definition`)
  - `path = "../lib"` → `../lib/.dotori` 파일 열기

- [ ] `compile_commands.json` 생성 (`CompileCommandsExporter`)
...
- [ ] LSP 서버 등록 (`extension.toml`)
  ...
  - 언어 정의: 파일 확장자 `.dotori`, 줄 주석 없음, 블록 주석 `(* ... *)`
