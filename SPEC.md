...
- [ ] `path` 의존성을 재귀적으로 탐색하여 전체 프로젝트 그래프 구성
- [ ] 토폴로지 정렬 → 빌드 순서 결정
- [ ] 순환 의존성 감지 + 명확한 오류 출력
- [ ] DAG 병렬화: 의존 관계 없는 프로젝트 동시 빌드
...
- [ ] `MsvcDriver`, `ClangDriver`, `EmscriptenDriver`
- [ ] `MsvcLinker`, `LldLinker`, `AppleLinker`
- [ ] Glob 확장기
- [ ] DAG 생성 (파일 단위: `.cpp` → `.obj`, `.cppm` → `.bmi`)
- [ ] C++ Modules 스캔 + BMI 순서 결정
- [ ] PCH 플래너 (Modules와 PCH 동시 사용 시 경고)
- [ ] Unity Batcher (Modules 소스 자동 제외)
- [ ] 증분 빌드 해시 DB (`.dotori-cache/hashes.db`)
- [ ] 병렬 빌드 (`Channel<BuildJob>`)
- [ ] 단일 파일 빌드 (`--file`)
...
Phase 3 — 생태계
22. [ ] 레지스트리 서버, VS Code, Zed, CLion 확장 구현
```
