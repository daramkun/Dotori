...
프로젝트 파일에 `option` 블록으로 선택적 빌드 기능을 선언하고, CLI 플래그로 켜고 끌 수 있는 기능.

#### 설계

- `option <name> { default = true/false; defines { }; dependencies { } }` 블록을 `project` 내에 선언
- 옵션 이름은 기존 `[atom]` 조건 블록 시스템과 동일하게 활용 가능 (`[옵션명]` 조건 블록 자동 연동)
- 옵션은 `default` 값이 기본 활성 여부, CLI 플래그로 재정의 가능
- 활성된 옵션은 환경변수 `DOTORI_OPTION_<NAME>` (대문자, `-`→`_`)로 노출됨
...
project MyApp {
    ...
    # 기존 조건 블록에서 옵션 이름을 atom으로 사용 가능
    [simd] {
        compile-flags { "-mavx2" }
...
```bash
dotori build --simd                  # default=false 옵션 활성화
dotori build --no-simd               # default=true 옵션 비활성화
dotori build --simd --experimental   # 여러 옵션 동시 지정
```

프로젝트에 선언되지 않은 `--X` 플래그는 오류로 처리됩니다.

#### 옵션 속성

| 속성 | 타입 | 필수 | 설명 |
|------|------|------|------|
| `default` | bool | ✅ | 기본 활성 여부 |
| `defines` | string 목록 | — | 옵션 활성 시 추가할 정의 |
...
| 변수명 | 예시 | 설명 |
|--------|------|------|
| `DOTORI_OPTION_<NAME>` | `DOTORI_OPTION_SIMD=1` | 이름 대문자화, `-`→`_` 변환. 비활성 시 `0` |
...
- [ ] `BuildContext.ScanOptions()` 헬퍼 추가
- [ ] `BuildContext.MakeTargetContext()` 옵션 환경변수 주입
- [ ] `BuildCommand` 동적 `--X` / `--no-X` 처리
- [ ] 단위 테스트 (파서, 플래트너, 환경변수)
- [ ] `docs/dotori-file.md`, `docs/cli.md` 문서 업데이트
...
프로젝트 구조:
- `Dotori.LanguageServer/` — `Dotori.Cli`에 통합된 LSP 라이브러리 (별도 바이너리 없음)
- `dotori lsp` CLI 서브커맨드로 시작 (stdio transport)
- IDE 플러그인은 `dotori lsp` 하나만 알면 됨 — CLI와 항상 같은 버전
